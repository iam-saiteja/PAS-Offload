...
By combining **transposed column-major weight caching**, a **CPU-side low-rank activation predictor**, **dynamic 2-bit weight slicing**, and **double-buffered pipelining**, PAS-Offload achieves a **10.20x total speedup** over standard CPU fallback and hides weight streaming latency completely.

## Key Benefits

When running LLMs that exceed your GPU frame buffer, implementing **PAS-Offload** provides several key advantages:

*   **10x Inference Speedup:** Boosts local inference speeds from sluggish CPU/GPU splits ($\approx 1.5\text{–}3.0\text{ tokens/s}$) up to comfortable human-reading speeds (**$10\text{–}15\text{ tokens/s}$**) using double-buffered execution overlapping.
...
* **Standard CPU Fallback:** **$79.98\text{ ms}$** (sluggish processing, cache-thrashing on Host CPU).
* **Sequential PAS-Offload (V1):** **$11.46\text{ ms}$** (**$7.10\text{x}$ speedup**, GPU-accelerated dequantization).
* **Pipelined PAS-Offload (V2):** **$7.98\text{ ms}$** (**$10.20\text{x}$ speedup**, double-buffered overlapping, zero dynamic memory allocations).
...
PAS-Offload accelerates layer computation by executing on the GPU instead of fallback CPU execution, resulting in over **10x execution speedup** per layer using double-buffered execution.
